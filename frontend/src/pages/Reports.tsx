...
export default function Reports() {
  ...
  const { data: overview, isLoading: loadingOverview } = useQuery<OverviewReport>({
    queryKey: ["reports_overview", range],
    queryFn: async () => {
      const res = await fetch(`${API_BASE}/reports/overview?${params.toString()}`, { credentials: "include" });
      if (!res.ok) throw new Error("Failed to load overview report");
      return res.json();
    },
  });

  const { data: sla, isLoading: loadingSla } = useQuery<SlaReport>({
    queryKey: ["reports_sla", range],
    queryFn: async () => {
      const res = await fetch(`${API_BASE}/reports/sla?${params.toString()}`, { credentials: "include" });
      if (!res.ok) throw new Error("Failed to load SLA report");
      return res.json();
    },
  });

  const { data: agents = [], isLoading: loadingAgents } = useQuery<AgentReport[]>({
    queryKey: ["reports_agents", range],
    queryFn: async () => {
      const res = await fetch(`${API_BASE}/reports/agents?${params.toString()}`, { credentials: "include" });
      if (!res.ok) throw new Error("Failed to load agent report");
      return res.json();
    },
  });

  const { data: drilldown, isLoading: loadingDrilldown } = useQuery<DrilldownResponse>({
    queryKey: ["reports_drilldown", drilldownType, range],
    queryFn: async () => {
      const drillParams = new URLSearchParams({ ...range, type: drilldownType || "created" });
      const res = await fetch(`${API_BASE}/reports/drilldown?${drillParams.toString()}`, { credentials: "include" });
      if (!res.ok) throw new Error("Failed to load drill-down report");
      return res.json();
    },
    enabled: !!drilldownType,
  });
  ...
  const downloadCsv = async () => {
    ...
    const drillParams = new URLSearchParams({ ...range, type: drilldownType, format: "csv" });
    const res = await fetch(`${API_BASE}/reports/drilldown?${drillParams.toString()}`, { credentials: "include" });
    if (!res.ok) return;
  ...
  return (
    <div className="w-full px-6 md:px-10 py-8 font-sans text-ink bg-canvas">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 mb-8">
        ...
        <div className="border border-black/10 rounded-xl bg-surface/10 p-3 flex flex-wrap items-end gap-2">
          <button onClick={() => setRange(getPresetRange(1))} className="px-3 py-2 rounded-lg text-xs font-semibold bg-canvas border border-black/10 hover:bg-surface">
            Today
          </button>
          <button onClick={() => setRange(getPresetRange(7))} className="px-3 py-2 rounded-lg text-xs font-semibold bg-canvas border border-black/10 hover:bg-surface">
            7 Days
          </button>
          <button onClick={() => setRange(getPresetRange(30))} className="px-3 py-2 rounded-lg text-xs font-semibold bg-canvas border border-black/10 hover:bg-surface">
            30 Days
          </button>
              ...
              {kpis.map((kpi) => {
                ...
                return (
                  <button
                    ...
                    className="border border-black/10 rounded-xl bg-canvas p-4 text-left shadow-sm hover:shadow-md hover:border-black/20 transition-all disabled:hover:shadow-sm disabled:cursor-default"
                  >
                    <div className="flex items-center justify-between mb-4">
                      <Icon className="size-4 text-muted-foreground" />
                      {meta.drilldown && <Search className="size-3.5 text-muted-foreground" />}
                    </div>
          ...
          <section className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                ...
                return (
                  <button key={item.label} onClick={() => setDrilldownType(item.type)} className="border border-black/10 rounded-xl bg-canvas p-4 text-left shadow-sm hover:shadow-md hover:border-black/20 transition-all">
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={[
                      ...
                      <YAxis allowDecimals={false} tick={{ fontSize: 10 }} />
                      <Tooltip />
                      <Bar dataKey="met" fill="#16a34a" radius={[4, 4, 0, 0]} />
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={sla.historical.breachTrend}>
                        ...
                        <YAxis allowDecimals={false} tick={{ fontSize: 10 }} />
                        <Tooltip />
                        <Line type="monotone" dataKey="breached" stroke="#dc2626" strokeWidth={2} dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                ) : (
                    ...
                    <tr>
                      ...
                      {[
                        ...
                        <th key={key} className="px-4 py-3">
                          <button onClick={() => setAgentSort(key as keyof AgentReport)} className="hover:text-ink">{label}</button>
                        </th>
                      ))}
                    </tr>
                  </thead>
      ...
      {drilldownType && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex justify-end">
          <div className="w-full max-w-4xl h-full bg-canvas shadow-xl border-l border-black/10 flex flex-col">
            <div className="p-5 border-b border-black/10 flex items-center justify-between gap-4">
              <div>
                ...
                <h3 className="font-serif text-2xl mt-1">{drilldownType.replace(/_/g, " ")}</h3>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={downloadCsv} className="px-3 py-2 rounded-lg bg-ink text-canvas text-xs font-semibold flex items-center gap-1.5">
                  <Download className="size-3.5" />
                  CSV
                </button>
                <button onClick={() => setDrilldownType(null)} className="size-9 rounded-lg border border-black/10 grid place-items-center hover:bg-surface">
                  <X className="size-4" />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-auto">
              {loadingDrilldown ? (
                ...
                <table className="w-full text-left text-xs">
                  <thead className="sticky top-0 bg-surface text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
                    <tr>
                  ...
                  <tbody className="divide-y divide-black/5">
                    {drilldown.rows.map((ticket) => (
                      <tr key={ticket.id} className="hover:bg-surface/20">
                        ...
                        <td className="px-4 py-3 font-semibold max-w-sm">
                          ...
                          <span className="block text-[10px] text-muted-foreground mt-1">{ticket.priority} priority</span>
                        </td>
                        <td className="px-4 py-3">{ticket.clientName || "N/A"}</td>
                        <td className="px-4 py-3">{ticket.assigneeName || "Unassigned"}</td>
                        <td className="px-4 py-3">{ticket.status}</td>
                        <td className="px-4 py-3">{ticket.slaState}</td>
